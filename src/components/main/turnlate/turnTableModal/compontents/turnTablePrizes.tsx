    ...
    return (
        <>
            <div className="dailyMainContainerType5">
                {prizeList?.content?.length === 0 || prizeList?.length === 0 || prizeList.code === "ERR_BAD_RESPONSE" || prizeList.code === "ERR_BAD_REQUEST" || prizeList.success === false || prizeList?.content === "" || prizeList === "" ? <NoDataV2 height={"100%"} /> :
                    <>
                            ...
                            <div className="dailyContainer" key={index}>
                                <div className="imageLabelBox">
                                    ...
                                    <div className="labelBox">
                                        <span className="lableTitle" style={{ color: color.text }}>{timestampToTime(value?.createDatetime)}</span>
                                        <span className="lableTitle" style={{ color: color.text }}>{value?.username} <span style={value?.status == 1 ? { color: "#EA4E3D", fontStyle: "italic" } : { color: "#04BE02", fontStyle: "italic" }}>{value?.status == 1 ? t("ts699", { ns: "ts" }) : value?.status == 2 ? t("ts848", { ns: "ts" }) : "-"}</span> </span>
                                    </div>
                                </div>
