    ...
    <PageLayout title="Invite">
        <div class="flex flex-col gap-[.2rem] p-[.2rem] w-screen">
            ...
            <div class="flex flex-col h-[calc(100vh-4.3rem)] overflow-auto">
                <div v-for="( commissionValue, indexes) in commissionData?.content?.rows" :key=indexes
                    ...
                    <div class="grid grid-cols-2 gap-[.2rem]">
                        <div class="flex flex-col gap-[.1rem]">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Time of Settlement {{ commissionValue?.statDateStr }}</span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">My Income {{ commissionValue?.oriBetNum }}</span>
                            </div>
                        </div>
                        <div class="flex flex-col gap-[.1rem] justify-end">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Type Slot</span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Commission</span>
                                <span class="flex items-center text-[#ffaa09] text-[.2rem]">{{ commissionValue?.money ?
                                    commissionValue?.money : "0" }}</span>
                            </div>
                        </div>

                    </div>
                </div>
                <div v-if="commissionData?.content?.rows?.length == 0"
