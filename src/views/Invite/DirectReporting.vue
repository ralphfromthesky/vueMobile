<template>
    <PageLayoutTab title="Invite">
        <div class="flex flex-col gap-[.2rem] w-screen h-[calc(100vh-1.7rem)] bg-[#05309F] text-white p-[.2rem]">
            <div class="flex gap-[.2rem] w-screen">
                <div
            ...
            <div class="flex flex-col h-[calc(100vh-2.8rem)] overflow-auto">
                <div v-for="(reportValue, indexes) in reportData?.content?.rows" :key=indexes
                    ...
                    <div
                        ...
                        <span class="text-[.15rem] text-white">V{{ reportValue.level }}</span>
                    </div>
                    <div class="grid grid-cols-2 gap-[.2rem]">
                        <div class="flex flex-col gap-[.1rem]">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ reportValue.promotionCode ? reportValue.promotionCode : "-" }}</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]"><img
                                        src="/copyIcons/Cartier_copy.png" alt="" class="w-[.3rem]"></span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Initial Deposit:</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ reportValue.firstDeposit
                                    }}</span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Current:</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ reportValue.onlineStatus
                                    }}</span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">State:</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ reportValue.status }}</span>
                            </div>
                        </div>
                        <div class="flex flex-col gap-[.1rem]">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Registration time:</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ reportValue.createDatetime
                                    }}</span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Login last time:</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ reportValue.lastLoginDatetime ? reportValue.lastLoginDatetime : "-"  }}</span>
                            </div>
            ...
            <div v-if="reportData?.content?.rows?.length == 0"
