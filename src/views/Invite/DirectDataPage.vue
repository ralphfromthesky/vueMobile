    ...
    <PageLayoutTab title="Invite">
        <div class="flex flex-col gap-[.2rem] w-screen h-[calc(100vh-1.7rem)] bg-[#05309F] text-white p-[.2rem]">
            ...
            <div class="flex flex-col h-[calc(100vh-2.8rem)] overflow-auto">
                <div v-for="(directdataValue, indexes) in DirectData?.content?.rows" :key=indexes
                    ...
                    <div
                        ...
                        <span class="text-[.15rem] text-white">V{{ directdataValue.level }}</span>
                    </div>
                    <div class="grid grid-cols-2 gap-[.2rem]">
                        <div class="flex flex-col gap-[.1rem]">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{directdataValue.promotionCode ? directdataValue.promotionCode : "-" }}</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]"><img
                                        src="/copyIcons/Cartier_copy.png" alt="" class="w-[.3rem]"></span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Initial Deposit: {{ directdataValue.firstDeposit ? directdataValue.firstDeposit : "No" }}</span>
                            </div>
                            <div class="flex flex-col gap-[.1rem] justify-end">
                                <div class="flex items-center gap-[.1rem]">
                                    <span class="flex items-center text-[#fff] text-[.2rem]">current:</span>
                                    <span class="flex items-center text-[#fff] text-[.2rem]">{{ directdataValue.onlineStatus ? directdataValue.onlineStatus : "Offline"}}</span>
                                </div>
                            </div>
                            <div class="flex flex-col gap-[.1rem] justify-end">
                                <div class="flex items-center gap-[.1rem]">
                                    <span class="flex items-center text-[#fff] text-[.2rem]">state:</span>
                                    <span class="flex items-center text-[#04be02] text-[.2rem]">{{ directdataValue.status}}</span>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-col gap-[.1rem] justify-end">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Registration time:</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ directdataValue.createDatetime ? directdataValue.createDatetime : "-" }}</span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Login last time:</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ directdataValue.lastLoginDatetime ? directdataValue.lastLoginDatetime : "-" }}</span>
                            </div>
                        </div>

                    </div>
                </div>
                <div v-if="DirectData?.content?.rows?.length == 0"
...
const DirectDataPage = useMutation({
    mutationFn: (payload) => axiosPost2('api/native/v2/getDirectData.do', payload),
    onSuccess: (data) => {
