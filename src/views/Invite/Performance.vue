    ...
    <PageLayout title="Invite">
        <div class="w-screen">
            ...
            <div class="flex flex-col h-[calc(100vh-4.3rem)] overflow-auto p-[.2rem]">
                <div v-for="( incomeValue, indexes) in incomeData?.content?.rows" :key=indexes
                    ...
                    <div
                        ...
                        <span class="text-[.15rem] text-white">V{{ incomeValue?.level }}</span>
                    </div>
                    <div class="grid grid-cols-2 gap-[.2rem]">
                        <div class="flex flex-col gap-[.1rem]">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ incomeValue?.promotionCode ?
                                    incomeValue?.promotionCode : "-" }}</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]"><img
                                        src="/copyIcons/Cartier_copy.png" alt="" class="w-[.3rem]"></span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ incomeValue?.statDate ? DateToStr(incomeValue?.statDate ) : "-" }}</span>
                            </div>
                        </div>
                        <div class="flex flex-col gap-[.1rem] justify-end">
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Team Performance</span>
                            </div>
                            <div class="flex items-center gap-[.1rem]">
                                <span class="flex items-center text-[#fff] text-[.2rem]">Commission Contribution</span>
                                <span class="flex items-center text-[#fff] text-[.2rem]">{{ incomeValue?.money ?
                                    incomeValue?.money : "0" }}</span>
                            </div>
                        </div>

                    </div>
                </div>
                <div v-if="incomeData?.content?.rows?.length == 0"
                ...
                <div class="flex gap-[.2rem]">
                    <div class="flex flex-col gap-[.1rem] w-fit leading-none">
                        <div class="flex gap-[.5rem] items-center justify-between">
                            <div class="flex items-center"><span class="text-[.2rem] text-white">Number of Direct
                                    Bettors</span></div>
                            <div class="flex items-center"><span class="text-[.2rem] text-white">{{ incomeData?.content?.aggsData?.directSubNum ? incomeData?.content?.aggsData?.directSubNum : 0 }}</span></div>
                        </div>
                        <div class="flex gap-[.5rem] items-center justify-between">
                            <div class="flex items-center text-nowrap"><span class="text-[.2rem] text-white">Direct
                                    Betting</span></div>
                            <div class="flex items-center"><span class="text-[.2rem] text-white">{{ incomeData?.content?.aggsData?.directSubBetNum ? incomeData?.content?.aggsData?.directSubBetNum : 0 }}</span></div>
                        </div>
                        <div class="flex gap-[.5rem] items-center justify-between">
                            <div class="flex items-center text-nowrap"><span class="text-[.2rem] text-white">Direct
                                    Commission</span></div>
                            <div class="flex items-center"><span class="text-[.2rem] text-white">{{ incomeData?.content?.aggsData?.directSubMoney ? incomeData?.content?.aggsData?.directSubMoney : 0 }}</span></div>
                        </div>
                    </div>
                </div>
                <div class="flex gap-[.2rem]">
                    <div class="flex flex-col gap-[.1rem] leading-none">
                        <div class="flex gap-[.5rem] items-center justify-between">
                            <div class="flex items-center "><span class="text-[.2rem] text-white">Commission
                                    Contribution</span></div>
                            <div class="flex items-center"><span class="text-[.2rem] text-white">{{ AllData?.content?.aggsData?.otherSubNum ? AllData?.content?.aggsData?.otherSubNum : 0 }}</span></div>
                        </div>
                        <div class="flex gap-[.5rem] items-center justify-between">
                            <div class="flex items-center text-nowrap"><span class="text-[.2rem] text-white">Other
                                    Bets</span></div>
                            <div class="flex items-center"><span class="text-[.2rem] text-white">{{ AllData?.content?.aggsData?.otherSubBetNun ? AllData?.content?.aggsData?.otherSubBetNun : 0 }}</span></div>
                        </div>
                        <div class="flex gap-[.5rem] items-center justify-between">
                            <div class="flex items-center text-nowrap"><span class="text-[.2rem] text-white">Other
                                    Commissions</span></div>
                            <div class="flex items-center"><span class="text-[.2rem] text-white">{{ AllData?.content?.aggsData?.otherSubMoney ? AllData?.content?.aggsData?.otherSubMoney : 0 }}</span></div>
                        </div>
                    </div>
                </div>
            </div>
...
const Income = useMutation({
    mutationFn: (payload) => axiosPost2('api/native/v2/getUserIncomePage.do', payload),
    onSuccess: (data) => {
...
const acct = [{ account: "neil", DateStart: "2024/05/24", income: "1000" }, { account: "atong", DateStart: "2024/05/24", income: "1000" }, { account: "mark", DateStart: "2024/05/24", income: "1000" }]
</script>