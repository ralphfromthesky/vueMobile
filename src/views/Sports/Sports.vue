      ...
      <div class="h-screen w-screen" v-if="hideMain">
        <div>
          <div class="p-[.3rem] text-white flex justify-between font-[1rem]">
            <router-link to="/">
              <span class="text-[.5rem]"><</span></router-link>
            <span class="text-[.5rem]">Sports</span>
            <span></span>
          </div>
...
//   const { gamesData, getGame } = getGamesTab();
  ...
  const { refetch: refetchBbin } = useQuery({
    ...
    queryFn: () =>
      axiosGet2(
        "/api/native/v2/get_game_datas_v2.do?gameType=tysbSport&lan=en&pageSize=30&pageIndex=1"
      ),
    enabled: false,
    select: (data) => {
      gameTabs.value = data;
  ...
  const { refetch: refetchGetGame } = useQuery({
    queryKey: ["getGame"],
    queryFn: () => axiosGet2("/api/native/v2/getGame2.do?type=0&limitNum=50&lan=en"),
    select: (data) => {gamesData.value = data},
