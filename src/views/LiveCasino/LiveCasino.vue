    ...
    <div class="h-screen w-screen" v-if="hideMain">
      <div>
        <div class="p-[.3rem] text-white flex justify-between font-[1rem]">
          ...
          <span class="text-[.5rem]">Live Casino</span>
          <span></span>
        </div>
...
const { refetch: refetchBbin } = useQuery({
  ...
  queryFn: () =>
    axiosGet2(
      "/api/native/v2/get_game_datas_v2.do?gameType=agLive&lan=en&pageSize=30&pageIndex=1"
    ),
  enabled: false,
  select: (data) => {
    gameTabs.value = data;
...
const { refetch: refetchGetGame } = useQuery({
  queryKey: ["getGame"],
  queryFn: () =>
    axiosGet2("/api/native/v2/getGame2.do?type=1&limitNum=50&lan=en"),
  select: (data) => {
